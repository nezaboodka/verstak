...
export function getFractionCount(isHorizontal: boolean, children: Array<RxNode<ElImpl>>, vector: number, index: number, force: boolean = false): number {
  ...
  for (const i of indexes(vector, index)) {
    const growth = (isHorizontal ? children[i].element.width.growth : children[i].element.height.growth) ?? 0
        ...
        for (let priority = 0; priority < priorities.length; priority++) {
          ...
          do {
            ...
            for (const i of indexes(vector, index)) {
              ...
              const g = isHorizontal ? (child.element.width.growth ?? 0) : (child.element.height.growth ?? 0)
