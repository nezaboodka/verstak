...
import { ElImpl } from "../core/ElImpl.js"
  ...
  for (let priority = 0; priority < priorities.length; priority++) {
    ...
    do {
      ...
      for (const i of indexes(vector, sign * index)) {
        ...
        const strength = isHorizontal ? (child.element.stretchingStrengthHorizontally ?? 1) : (child.element.stretchingStrengthVertically ?? 1)
